...
class P256PublicKeyVerifierTest extends WordSpec with EitherValues with Matchers {
  "P256PublicKeyVerifier" should {
    "successfully convert valid public key with a 30 byte x" in {
      ...
      val publicKey = P256PublicKeyVerifier(x, y).toEither.right.value
    ...
    "fail for negative number which isn't on the curve" in {
      ...
      val error = P256PublicKeyVerifier(x, y).toEither.left.value
